# Niu-Dash — Project AGENTS.md

**Lokasi:** `Production/niu-dash/`
**Stack:** Single-file HTML/CSS/JS Vanilla, PWA
**Remote:** `github.com/Niumination/niu-dash`
**Deploy:** 🟢 GH Pages — v2.17.0 (Redesign "Neo-Nexus") | **Audit:** 27/27 ✅
**HEAD:** 40fdc46 (clean — latest at Production/) | **Last Push:** 22 Jun 2026 (moved to Production/)

## Overview

Niu-Dash adalah dark web glitch dashboard untuk memonitor ekosistem Niumination. Single-file app (index.html) dengan PWA support, GitHub API integration, dan release tracking.

## Audit Status

27/27 ALL FIXED ✅ — Fase 1-7 selesai:
- Critical (3): Unlisted-add persist, overlay bug, PWA icon ✅
- High (7): Rate limit, font, categorization, commit cache, particles, README, font-size ✅
- Medium+ (12): RenderFeed, SW caching, CSP, path absolut, dll ✅
- Fix Batch (5): Pagination, renderFeed args, _gT cleanup, KANBAN_API_URL config, SW root cache ✅

## Tasks

Lihat `BACKLOG.md` untuk task detail.
